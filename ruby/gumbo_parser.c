...
static VALUE _name(GumboElement *node) {
  if (!TAGS) {
    ...
    // defined *after* the GumboC class is.
    VALUE GumboParser = rb_const_get(rb_cObject, rb_intern("GumboParser"));
    TAGS = rb_const_get(GumboParser, rb_intern("TAGS"));
    Unknown = NUM2INT(rb_const_get(GumboParser, rb_intern("Unknown")));
  }
...
// Initialize the GumboC class and fetch constants we will use later
void Init_gumboc() {
  ...
  // define GumboC class with a singleton parse method
  VALUE Gumbo = rb_define_class("GumboC", rb_cObject);
